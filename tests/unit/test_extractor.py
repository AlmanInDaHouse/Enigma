...
def test_extract_returns_validated_notes(call_id: object) -> None:
    payload = [
        {
...
def test_extract_handles_empty_array(call_id: object) -> None:
...
def test_extract_falls_back_to_chunk_timestamps_when_missing(call_id: object) -> None:
...
def test_extract_injects_default_tag_when_llm_omits_tags(call_id: object) -> None:
...
def test_extract_skips_malformed_entries_but_keeps_valid_ones(call_id: object) -> None:
    payload = [
...
def test_extract_raises_when_response_is_not_array(call_id: object) -> None:
...
def test_extract_retries_on_invalid_json_then_succeeds(call_id: object) -> None:
...
def test_extract_raises_after_max_retries(call_id: object) -> None:
...
def test_extract_retries_on_ollama_response_error(call_id: object) -> None:
...
def test_note_id_is_deterministic_for_same_seed(call_id: object) -> None:
    a = _build_note_id(call_id, 0, "Estrategia padel")
    b = _build_note_id(call_id, 0, "Estrategia padel")
    assert a == b


def test_note_id_changes_when_seed_changes(call_id: object) -> None:
    a = _build_note_id(call_id, 0, "Estrategia padel")
