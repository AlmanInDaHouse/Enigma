...
Eres un compañero de pensamiento creativo. A partir de una llamada de equipo,
EXPANDES las ideas que se trataron: no resumes lo dicho — propones lo que NO
se ha dicho todavía pero merece la pena explorar.
...
REGLAS:
1. `analogies`: paralelismos con otros dominios, problemas o productos que
   iluminen lo tratado desde otro ángulo.
2. `next_steps`: acciones o experimentos concretos que el equipo podría dar a
   continuación, más allá de lo ya decidido en la llamada.
3. `open_questions`: preguntas abiertas y relevantes que la llamada deja sin
   responder y convendría plantearse.
4. `risks`: riesgos, puntos ciegos o supuestos frágiles que no se mencionaron.
5. Cada item: una frase concisa y autocontenida, en español. Sé específico;
   evita las obviedades y las frases de relleno.
6. Aporta entre 2 y 5 items por categoría. Si una categoría no da para nada
   útil, devuélvela como lista vacía (`[]`).
"""


BRAINSTORM_USER_TEMPLATE = """\
Título de la llamada: {call_title}
Idioma: {language}
...
def build_brainstorm_messages(
    ...
    *,
    ...
    """Construye los `messages` para el brainstorming de una llamada (Ollama, JSON).
    ...
    Returns:
        Lista de dos dicts: `{"role": "system", ...}` y `{"role": "user", ...}`.
    """
    user_content = BRAINSTORM_USER_TEMPLATE.format(
        call_title=call_title,
        language=language,
        transcript_text=transcript_text,
    )
